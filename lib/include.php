...
if (! function_exists(__NAMESPACE__ . '\register2Dot0Dot1Dot8')) {
    ...
    add_action('plugins_loaded', __NAMESPACE__ . '\register2Dot0Dot1Dot8', -200, 0);

    function register2Dot0Dot1Dot8()
    {
        if (! interface_exists('PublishPress\Psr\Container\ContainerInterface')) {
            $versions = Versions::getInstance();
            $versions->register('2.0.1.8', __NAMESPACE__ . '\initialize2Dot0Dot1Dot8');
        }
    }

    function initialize2Dot0Dot1Dot8()
    {
        require_once __DIR__ . '/autoload.php';
        
        if (! defined('PUBLISHPRESS_PSR_CONTAINER_VERSION')) {
            define('PUBLISHPRESS_PSR_CONTAINER_VERSION', '2.0.1.8');
        }
        
        do_action('publishpress_psr_container_2Dot0Dot1Dot8_initialized');
    }
}
