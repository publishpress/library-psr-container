...
if (! function_exists(__NAMESPACE__ . '\register2Dot0Dot1Dot7')) {
    ...
    add_action('plugins_loaded', __NAMESPACE__ . '\register2Dot0Dot1Dot7', -200, 0);

    function register2Dot0Dot1Dot7()
    {
        if (! interface_exists('PublishPress\Psr\Container\ContainerInterface')) {
            $versions = Versions::getInstance();
            $versions->register('2.0.1.7', __NAMESPACE__ . '\initialize2Dot0Dot1Dot7');
        }
    }

    function initialize2Dot0Dot1Dot7()
    {
        require_once __DIR__ . '/autoload.php';
        
        if (! defined('PUBLISHPRESS_PSR_CONTAINER_VERSION')) {
            define('PUBLISHPRESS_PSR_CONTAINER_VERSION', '2.0.1.7');
        }
        
        do_action('publishpress_psr_container_2Dot0Dot1Dot7_initialized');
    }
}
