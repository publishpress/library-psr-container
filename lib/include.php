...
if (! function_exists(__NAMESPACE__ . '\register2Dot0Dot2Dot0')) {
    ...
    add_action('plugins_loaded', __NAMESPACE__ . '\register2Dot0Dot2Dot0', -200, 0);

    function register2Dot0Dot2Dot0()
    {
        if (! interface_exists('PublishPress\Psr\Container\ContainerInterface')) {
            $versions = Versions::getInstance();
            $versions->register('2.0.2.0', __NAMESPACE__ . '\initialize2Dot0Dot2Dot0');
        }
    }

    function initialize2Dot0Dot2Dot0()
    {
        require_once __DIR__ . '/autoload.php';

        if (! defined('PUBLISHPRESS_PSR_CONTAINER_VERSION')) {
            define('PUBLISHPRESS_PSR_CONTAINER_VERSION', '2.0.2.0');
        }

        do_action('publishpress_psr_container_2Dot0Dot2Dot0_initialized');
    }
}
